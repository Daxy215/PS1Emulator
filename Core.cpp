...
int main() {
    const std::string binFileName = "ROMS/Tetris X/Tetris X (Japan) (Track 1).bin"; // Replace with your actual file name
    
    // Open the binary file
    std::ifstream binFile(binFileName, std::ios::binary);
    if (!binFile.is_open()) {
        std::cerr << "Error opening file: " << binFileName << std::endl;
        return 1;
    }
    
    // Read the entire file into a vector (adjust buffer size as needed)
    std::vector<uint8_t> binData;
    binFile.seekg(0, std::ios::end);
    binData.resize(binFile.tellg());
    binFile.seekg(0, std::ios::beg);
    binFile.read(reinterpret_cast<char*>(binData.data()), binData.size());
    
    // Close the file
    binFile.close();
    
    std::bitset<8> binaryValue = 0;
    ...
    for (const auto& byte : binData) {
        ...
        binaryValue = std::bitset<8>(byte);
        
        // Add
        //if(binaryValue.to_ulong() == 0b100000) {
        //     std::bitset<8> op = binaryValue;
        //    
        //    // S, T, D
        //    std::bitset<8> s = ((op >> 21) & std::bitset<8>(0x1f));
        //    std::bitset<8> t = ((op >> 16) & std::bitset<8>(0x1f));
        //    std::bitset<8> d = ((op >> 11) & std::bitset<8>(0x1f));
        //    
        //    std::cout << "Binary value: " << op << " = " << s << " " << t << " " << d << '\n';
        //}
        
        //uint32_t virtualAddress = static_cast<uint32_t>(byte);
        //
        //if(virtualAddress >= heightest)
        //    heightest = virtualAddress;
        //
        //if (virtualAddress >= KSEG0_START && virtualAddress <= KSEG0_END) {
        //    kseg0Data.push_back(byte);
        //} else if (virtualAddress >= KSEG1_START && virtualAddress <= KSEG1_END) {
        //    kseg1Data.push_back(byte);
        //} else if (virtualAddress >= KSEG2_START && virtualAddress <= KSEG2_END) {
        //    kseg2Data.push_back(byte);
        //} else if (virtualAddress >= KUSEG_START && virtualAddress <= KUSEG_END) {
        //    kusegData.push_back(byte);
        //}
    }
